...
def fetch_pc(url, params=None) -> str:
...
def fetch_sp(url, params=None) -> str:
...
def save_calendar(year: int, month: int) -> list[str]:
    ...
    (SAVE_DIR_CAL / f"calendar_{year}_{month:02d}.html") \
...
def save_race_list(kaisai_date: str):
    ...
    (SAVE_DIR_LIST / f"race_list_{kaisai_date}.html") \
        .write_text(html, encoding="utf-8")
    ...
    (SAVE_DIR_ID / f"race_id_{kaisai_date}.txt") \
